...
# nombres viejos en español quedan como ALIAS para no romper runs en vuelo.
...
# Espejo del motor local: un turno que revienta (límite/crédito de la API, 500 del
# proveedor, presupuesto agotado, reinicio del conector) dejaba el run en `error` sin
# forma de seguir, y había que relanzar la tarea desde cero pagando de nuevo todo lo
# hecho. El estado igual está entero en run.json (frames con su transcript) + memoria
# y credenciales en disco: `resume` revive el MISMO run (mismo id y mismos events, así
# el historial no se corta) y `discard` lo cierra a mano. Solo el ÚLTIMO run es
# reanudable: los archivados no guardan `frames`.
...
def _last_run(ctx):
    """El run vivo en RAM o, si el conector se reinició, el de run.json (CON frames)."""
...
def _cached_messages(messages):
    """`messages` con `cache_control` en el ÚLTIMO bloque del ÚLTIMO mensaje.

    Copia el camino que toca en vez de mutar el transcript guardado: un marcador
    pegado en el historial se acumularía uno por turno y la API rechaza el request
    a partir del quinto (máximo 4 breakpoints)."""
    if not messages:
        return messages
    last = messages[-1]
...
class AnthropicChat:
    """Adapter de Anthropic CON prompt caching (2026-07-27). El orden de render es
    tools → system → messages, así que un breakpoint al final del system cachea
    tools + system juntos y otro al final del transcript cachea lo ya visto. Leer de
    caché sale 0,1× y escribir 1,25×: a partir del segundo turno del frame conviene."""
...
def make_adapter(ctx, node):
    ...
    if provider in CLI_PROVIDERS:
        raise OrchError(400, f"node «{node.get('titulo')}» uses a local CLI ('{provider}'): on "
                             "EXTERNAL connectors the heads are APIs only (decision M)")
...
def resource_tools(ctx, graph, node_id, author):
    ...
    for r in resources_of(graph, node_id):
        ...
        if not meta:
            notes.append(f"- {rid}: (project deleted or from another folder — do not use)")
            continue
        rtype = meta.get("type")
...
def build_system(ctx, graph, node, notes):
    """System prompt del agente. Va al MODELO → en inglés (doc 20 §L); el `rol` que
    escribió el humano se inyecta tal cual, en su idioma."""
    d = node.get("data") or {}
    nid = node["id"]
    partes = [
    ...
    partes.append(
        ...
        "verifiable summary). 5) Before editing a diagram look at its current JSON (view_tree) and respect "
        "its EXACT schema. 6) Write your answers in the same language the user/your caller writes to you in."
    )
    return "\n\n".join(partes)
...
def add_spend(run, node_id, usage):
    # cacheW/cacheR solo los reporta el adapter de Anthropic; en los otros quedan en 0
...
def _do_responder(ctx, graph, run, frame, mensaje):
    ...
    # la vuelta del token, registrada también del lado del que esperaba (sin esto el
    # timeline mostraba "responde" en el hijo y nada en el padre)
...
def _do_delegar(ctx, graph, run, frame, node, inp):
    ...
    # A QUIÉN delega, antes de crear los frames: el log del que delegaba solo decía
    # "esperando" y había que adivinar a quién llamó
...
def _loop(ctx):
    ...
    with cv:
        ...
        if run["status"] == "error":
            # queda trabajo a medio hacer ⇒ se puede RETOMAR (fase 13)
...
def get_state(ctx):
    ...
    if not run:
        ...
        if run and run.get("status") in ("running", "waiting_human", "paused"):
            run["status"] = "error"
            # el estado quedó en disco: se puede RETOMAR (fase 13)
            run["error"] = "the connector restarted during the run — you can resume it"
...
def answer(ctx, text, node_id=None):
    ...
    if not run or run["status"] not in ("running", "waiting_human"):
        raise OrchError(409, "there is no pending question (did the connector restart?)")
    with LOCK:
...
def resume(ctx, add_turns=None):
    ...
    (límite de la API, sin crédito, presupuesto agotado, conector reiniciado)."""
    live = RUNS.get(ctx["pid"])
...
def discard(ctx):
    """Cierra a mano un run muerto: deja de ofrecerse para reanudar. No borra nada
    del historial."""
    run = _last_run(ctx)
    if not run:
...
class OrchResumeBody(BaseModel):
    projectId: str
    addTurns: int | None = None       # estirar el presupuesto al reanudar (fase 13)
...
def orch_run(body: OrchRunBody, user: dict = Depends(require_admin)):
    def go(ctx):
        ...
        if not edge:
            raise OrchError(400, "the task is not connected to an agent (task arrow)")
...
def orch_resume(body: OrchResumeBody, user: dict = Depends(require_admin)):
    # pausado, o muerto en error con trabajo pendiente (fase 13): `addTurns` estira el
    # presupuesto si justamente fue eso lo que lo mató
    return _orch(body.projectId, user, lambda ctx: resume(ctx, body.addTurns))


@router.post("/orch/discard")
def orch_discard(body: OrchPidBody, user: dict = Depends(require_admin)):
    return _orch(body.projectId, user, discard)
