...
SYSTEM_PREAMBLE = (
    ...
    "vive en ./<Nombre>/tree.json (la carpeta lleva el NOMBRE del proyecto, no su "
    "id). Las skills (diagramind-format y la de cada tipo) "
    ...
    "Podés LEER cualquier otro proyecto (./<Nombre>/tree.json, buscalo en "
    "./index.json) para basarte en él; escribí en otro proyecto SOLO si el usuario "
    "te lo pide explícitamente.\n\n"
    "CREAR UN PROYECTO NUEVO: si te piden un diagrama NUEVO (en vez de modificar el "
    "foco), creá en ESTA carpeta un directorio con el NOMBRE del proyecto y adentro "
    "su `tree.json` con el esquema del tipo que elijas: ./<Nombre del proyecto>/tree.json. "
    "Nombre: letras, números, espacios, `-` y `_` (nada de `/`, `.` ni signos). Escribí "
    "el tree.json COMPLETO de una sola vez (válido desde el primer guardado). NO toques "
    "./index.json: lo mantiene la app, que detecta el proyecto nuevo en ~1 segundo, lo "
    "agrega a la lista del usuario y lo abre. Requiere que la app esté ABIERTA y el "
    "software local conectado (si no, el proyecto no sobrevive a la próxima "
    "sincronización): si sospechás que no lo está, avisale al usuario. NO crees "
    "proyectos tipo `editor` (necesitan que el usuario elija a mano una carpeta real "
    "del sistema). Después de crearlo, seguí trabajándolo ahí, no en el foco viejo.\n\n"
...
SKILLS = dict([
    ...
    _skill(
        ...
        "confinado:bool (default false; SOLO aplica a cabezas CLI: true = escribe "
        "ÚNICAMENTE a través de sus recursos editor, sin carpeta montada ni tools "
        "nativas de archivos — más seguro pero pierde Bash y no puede correr tests) }. "
        "El `titulo` es el NOMBRE del empleado.\n"
        "- `agData` → CONTEXTO ESTÁTICO: reglas, convenciones, aclaraciones. "
        "data={ contenido:string }. El `titulo` es el nombre del bloque. Se conecta "
        "agente→agData con la flecha `contexto` y su texto se inyecta EN EL SYSTEM de "
        "ese agente (siempre, no a demanda). Usalo para lo que TODOS los turnos deben "
        "respetar; para documentos largos usá un recurso editor.\n"
        ...
        "- `contexto` (agAgent→agData): le suma ese texto al system del agente. "
