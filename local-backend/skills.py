...
SKILLS = dict([
    ...
    _skill(
        ...
        "- `objcollection` → `data = { kind:\"list\"|\"queue\"|\"stack\"|\"tree\", elementClass: idDeClase|null }`. Serializa a array.\n"
        "- `objfetch` / `objburst` → `data = { method, url, headers:[{k,v}], webhook:{...} }`. `objburst` manda un request por cada elemento de la colección conectada.\n\n"
        "## Flechas (`kind` + `attrId`)\n"
