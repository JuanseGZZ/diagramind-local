...
SKILLS = dict([
    ...
    _skill(
        ...
        "`basic` shows ONLY its title, so a canvas built entirely out of `basic` nodes "
        "throws away all the content. These are every type this canvas has:\n\n"
        "| `type` | What it is | Its `data` |\n"
        "|---|---|---|\n"
        "| `md` | The card IS its text (Obsidian style). **This is the default for anything with content.** | none |\n"
        "| `basic` | Plain card: only the title is drawn. | none |\n"
        "| `grupo` | A SECTOR that groups other nodes. Read the section below. | none |\n"
        "| `web` | Embeds a page in an iframe; `contenido` is the URL. | none |\n"
        "| `class` | UML-style class box. | `campos:[\"+ field: type\"]`, `metodos:[\"+ method(): type\"]` |\n"
        "| `media` | Image or video; the URL goes in `data.src`. | `src`, `mediaType` (`image` or `video`) |\n"
        "| `shape` | A geometric shape that is a real node: it CONNECTS with arrows. | `shape`, `fill`, `stroke`, `strokeWidth` |\n"
        "| `whiteboard` | Hand drawing. Do NOT author one: leave it to the user. | `strokes`, `tool`, `color`, `lineWidth`, `mode` |\n\n"
        ...
        "Sizes that look right: `md` 300x220 (grow `alto` with the text), `grupo` "
        "380x280, `basic` 200x120.\n\n"
        "## Groups: READ THIS BEFORE GROUPING ANYTHING\n\n"
        "A group is a node with `\"type\":\"grupo\"`, and it holds its members BY "
        "GEOMETRY — there is no `parentId` field. A node belongs to the group whose "
        "rectangle contains the node's CENTER (`x + ancho/2`, `y + alto/2`); when two "
        "groups overlap, the smaller one wins. Therefore:\n\n"
        "- Make the group's rectangle big enough and put the children's centers INSIDE "
        "it: about 40 px of padding, 24 px between children. A group whose members are "
        "outside its rectangle is EMPTY, no matter what you named it.\n"
        "- Groups do NOT nest (v1 is flat) and are NOT arrow targets: draw the arrows "
        "between the real nodes, never to the group.\n"
        "- Moving a group drags everything inside it, and the app's explorer lists the "
        "group as a FOLDER holding its nodes.\n"
        ...
        "## The `contenido` of `md` and `grupo` is HTML, not Markdown\n\n"
        "It is the rich editor's HTML: `<p>`, `<ul><li>`, `<b>`, `<h3>`, "
        "`<a href=\"...\">`. A newline is NOT a line break: use `<p>` or `<br>`. "
        "(For `web` nodes `contenido` is a bare URL, and for the rest it is plain text.)\n\n"
        "## Editing\n"
